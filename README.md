...
sandbox, so they run on Linux only. The same diagram, with a dark variant, is
generated by the pipeline in [`website/diagrams`](website/diagrams).
...
│   ├── cli/            # broccoli CLI: scaffold, build, watch plugins
...
```bash
cargo run -p broccoli-cli -- plugin new my-plugin --full
just build-plugin plugins/my-plugin --install
```

The full walkthrough is in the docs site under Building plugins. The CLI is
documented in [`packages/cli/README.md`](packages/cli/README.md), the backend
SDK in [`packages/server-sdk`](packages/server-sdk), and the frontend SDK in
[`packages/web-sdk`](packages/web-sdk).
