...
/// Run batch evaluation for all test cases and return outcomes.
///
...
pub fn evaluate_all(
    ...
    // tc_id -> tc.score (max points per TC)
    let score_map: HashMap<i32, f64> = test_cases.iter().map(|tc| (tc.id, tc.score)).collect();
    ...
    let batch_id = match host.start_evaluate_batch(&batch_input) {
        ...
        Err(e) => {
            for tc in test_cases {
                ...
                insert_tc_result(host, submission_id, &outcome, &score_map)?;
...
fn insert_tc_result(
    ...
    submission_id: i32,
    outcome: &EvalOutcome,
    score_map: &HashMap<i32, f64>,
) -> Result<(), SdkError> {
    let tc_max = score_map.get(&outcome.test_case_id).copied().unwrap_or(0.0);
