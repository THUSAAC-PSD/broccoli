...
impl Broker for RedisBroker {
    ...
    async fn reject(
        ...
        if self
            ...
            .map_or(true, |config| config.retry_failed.unwrap_or(true))
        {
            // BROCCOLI VENDOR PATCH (see VENDOR-PATCHES.md): `metadata` is
            // `#[serde(skip)]`, so after a message has been consumed (HGETALL)
            // its in-memory `priority` metadata is ALWAYS None. Upstream then
            // `?`-returns "Missing priority" here -- but the message was already
            // LREM'd from the processing queue at the top of `reject`, so the
            // retry re-publish never happens and the message is SILENTLY LOST on
            // the first rejection. Fall back to the lowest priority (5) instead
            // of erroring, so a rejected message is always re-queued for retry.
            let priority = message
                .metadata
                .as_ref()
                .and_then(|m| m.get("priority"))
                .and_then(|v| match v {
                    MetadataTypes::String(priority) => priority.parse::<u8>().ok(),
    ...
    async fn size(
        ...
        if disambiguators.is_empty() {
            ...
            for disambiguator in disambiguators {
                ...
                let size: u64 = redis_connection.zcard(&sub_queue_name).await.map_err(|e| {
