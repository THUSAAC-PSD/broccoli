...
pub async fn client_from_url(
    ...
    let config = SurrealDBConnectionConfig {
        username: get_param_value(&url, "username").unwrap_or_else(|_| "root".to_string()),
        password: get_param_value(&url, "password").unwrap_or_else(|_| "root".to_string()),
        ns: get_param_value(&url, "ns").unwrap_or_else(|_| "test".to_string()),
        database: get_param_value(&url, "database").unwrap_or_else(|_| "test".to_string()),
    };
...
async fn add_record_to_queue(
    ...
    let _ = self::add_to_queue_index(
        db,
        queue_name,
        task_id,
        queue_record_id.clone(),
        ts,
        err_msg,
    )
    .await?;
    ...
    let mut retryable =
        RetriableSurrealDBResult::new(format!("{err_msg}:'{queue_name}': adding to queue"));
    while !retryable.is_done() {
        let result: Result<Option<InternalSurrealDBBrokerMessageEntry>, surrealdb::Error> =
            db.create(&queue_record_id).content(msg.clone()).await;
...
async fn add_to_queue_index(
    ...
    let qm: Option<InternalSurrealDBBrokerQueueIndex> = db
        ...
        .content(InternalSurrealDBBrokerQueueIndex {
            queue_id,
            timestamp: ts,
        })
        .await
...
pub async fn get_queued_transaction(
    ...
    let mut retryable =
        RetriableSurrealDBResult::new(format!("{err_msg}:'{queue_name}': transaction consume"));
    while !retryable.is_done() {
...
pub(crate) async fn get_queued_transaction_impl(
    ...
    match result {
        Ok(mut resp) => {
            let returned: Result<Vec<InternalSurrealDBBrokerMessage>, surrealdb::Error> =
...
async fn remove_from_queue_add_to_processed_transaction_impl(
    ...
    match result {
        Ok(mut resp) => {
            let returned: Result<Option<InternalSurrealDBBrokerMessageEntry>, surrealdb::Error> =
                resp.take(resp.num_statements() - 1);
            let transaction = resp.check(); //take(0 as usize);
            match transaction {
                Ok(_) => match returned {
...
pub(crate) async fn remove_from_queue_add_to_processed_transaction(
    ...
    let mut retryable =
        RetriableSurrealDBResult::new(format!("{err_msg}:'{queue_name}': consume transaction"));
    while !retryable.is_done() {
            ...
            match transaction {
                Ok(_) => match returned {
                    Ok(returned) => match returned {
                        Some(entry) => Ok(entry),
                        None => Err(BroccoliError::Broker(format!(
                            "'{queue_name}' Could not ack in transaction (nothing returned)"
                        ))),
                    },
                    Err(e) => Err(transaction_error(
                        &e,
                        format!(
                            "{err_msg}:'{queue_name}' Could not ack in transaction (response issue): {e}"
                        ),
                    )),
                },
                Err(e) => Err(transaction_error(
                    &e,
                    format!(
                        "{err_msg}:'{queue_name}' Could not ack in transaction (could not take): {e}"
                    ),
                )),
            }
        }
...
impl<T> RetriableSurrealDBResult<T> {
    ...
    {
        match result {
            Ok(r) => self.status = Some(Ok(r)),
            Err(e) => {
                if format!("{}", &e).contains("This transaction can be retried") {
                    tokio::time::sleep(tokio::time::Duration::from_millis(u64::from(self.retries)))
                        .await;
                    self.retries += 1;
                } else {
