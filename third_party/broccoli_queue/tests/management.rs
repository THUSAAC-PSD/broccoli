...
async fn test_queue_status_main_queue() {
    ...
    assert_eq!(
        status.processing, 0,
        "Non-existent queue should have 0 processing"
    );
    ...
    #[cfg(not(feature = "test-fairness"))]
    {
        ...
        assert_eq!(status.processing, 0, "No messages should be processing");
    ...
    let empty_result = queue.queue_status("".to_string(), None).await;
    assert!(
        empty_result.is_err(),
...
async fn test_queue_status_fairness_queue() {
    ...
    assert_eq!(
        status_nonexistent.name,
        format!("{}_job-999", test_topic_fairness)
    );
    assert_eq!(status_nonexistent.queue_type, QueueType::Fairness);
    assert_eq!(
        status_nonexistent.size, 0,
        "Non-existent disambiguator should have size 0"
    );
    assert_eq!(
        status_nonexistent.processing, 0,
        "Non-existent disambiguator should have 0 processing"
    );
    assert_eq!(
        status_nonexistent.failed, 0,
        "Non-existent disambiguator should have 0 failed"
    );
...
async fn test_queue_status_processing_and_failed() {
    ...
    assert_eq!(status.processing, 1, "One message should be processing");
...
async fn test_queue_status_specific_queue_lookup() {
    ...
    assert_eq!(
        nonexistent_status.name,
        format!("{}_nonexistent", base_name)
    );
    assert_eq!(
        nonexistent_status.size, 0,
        "Non-existent queue should have size 0"
    );
