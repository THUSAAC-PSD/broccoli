...
| 19  | Stuck-detector wide-net: raise `stuck_job_timeout_secs` default to 21600 (6h). Keep row-level recovery behind lease heartbeat staleness; later UP#43 makes old `Queued` backlog observability-only instead of per-row DLQ. **Builds on item 14f's `updated_at` fix**. | audit Phase 1b PR-E.                                                | `packages/server/src/dlq/stuck.rs`, `packages/common/src/config.rs`                                                                                                                     | items 14f, 18    | small  | Replaces the time-based primary recovery role; lease/steal is the primary path.                                                                            |
...
| #   | Title                                                                                                                                                                                                                                                                                 | Source                | File:line target                                                                                                                                                                 | Depends on                                                                                              | Effort | Rationale                                                                                                                                                                                                                   |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 36  | Add `Queued` to `SubmissionStatus` enum; make the stuck detector distinguish durable queued backlog from claimed recovery states.                                                                                                                                                     | audit Phase 3.        | `packages/common/src/submission_status.rs`, `packages/server/src/dlq/stuck.rs:17-21`                                                                                             | item 19 (so stuck-detector treats `Queued` distinctly)                                                  | small  | Additive enum variant; existing rows never contain it.                                                                                                                                                                      |
...
| 43  | Per-state stuck policy: observe old `Queued` backlog as aggregate dispatcher-health signal only; recover `Pending` rows with `owner_server_id IS NULL` after 5 min; keep `Compiling`/`Running` free of row-age limits and rely on lease heartbeat/steal. 6 h wide-net remains lease-staleness catch-all. | audit §1.11 + §3.7.                                                                  | `packages/server/src/dlq/stuck.rs`                                                                                                                   | items 36, 42 | small  | Prevents queued-backlog SystemError storms while still repairing orphaned post-claim rows and stale owned leases.                                                                        |
